...
find "/data/Ground_truth_Aim1_skylar/PathML Study/Beh_Posture_noldus" -name 'PathML*.csv' -print0 | xargs -0 \
    ...
    then filter '$Event_Type == "State start"' \
    then join -f task6_pathml.csv -j Observation -f /data/training_data/keyfiles/phase2/pathml/task1.csv -j Behavior \
...
for subtask in "${subtasks[@]}"; do
    find "/data/Ground_truth_Aim1_skylar/PathML Study/Beh_Posture_noldus" -name 'PathML*.csv' -print0 | xargs -0 \
        ...
        cut -f 'Observation,Time_Relative_sf,Duration_sf,Behavior,Event_Type' \
        then filter '$Event_Type == "State start"' \
        then join -f task6_pathml.csv -j Observation -f /data/training_data/keyfiles/phase2/pathml/task2.csv -j Behavior \
        then filter '$Task6 == "high"' \
        then put '$stop = $Time_Relative_sf + $Duration_sf' \
        then rename 'Time_Relative_sf,start' \
...
find "/data/Ground_truth_Aim1_skylar/PathML Study/Beh_Posture_noldus" -name 'PathML*.csv' -print0 | xargs -0 \
    ...
    then join -f task6_pathml.csv -j Observation -f /data/training_data/keyfiles/phase2/pathml/task3.csv -j Behavior \
    then filter '$Task6 == "high"' \
    then put '$stop = $Time_Relative_sf + $Duration_sf' \
    then rename 'Time_Relative_sf,start' \
...
for subtask in "${subtasks[@]}"; do
    find "/data/Ground_truth_Aim1_skylar/PathML Study/Beh_Posture_noldus" -name 'PathML*.csv' -print0 | xargs -0 \
        ...
        then join -f task6_pathml.csv -j Observation -f /data/training_data/keyfiles/phase2/pathml/task4.csv -j Behavior \
        then filter '$Task6 == "high"' \
        then put '$stop = $Time_Relative_sf + $Duration_sf' \
        then rename 'Time_Relative_sf,start' \
        then cut -f "Observation,Task${subtask},start,stop" \
...
for subtask in "${subtasks[@]}"; do
    find "/data/Ground_truth_Aim1_skylar/PathML Study/Location_Quality_noldus" -name 'PathML*.csv' -print0 | xargs -0 \
        ...
        cut -f 'Observation,Time_Relative_sf,Duration_sf,Behavior,Event_Type' \
        then filter '$Event_Type == "State start"' \
        then join -f task6_pathml.csv -j Observation -f /data/training_data/keyfiles/phase2/pathml/task5.csv -j Behavior \
        then filter '$Task6 == "high"' \
        then put '$stop = $Time_Relative_sf + $Duration_sf' \
        then rename 'Time_Relative_sf,start' \
        then cut -f "Observation,Task${subtask},start,stop" \
